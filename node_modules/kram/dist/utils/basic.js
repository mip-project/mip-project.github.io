 ...
 * @return {Object} 子集
 */
function subset(obj, keys, ignore) {
    if (ignore) {
        keys = exclude(Object.keys(obj), keys);
    }

    return keys.reduce(function (res, key) {
        return set(res, key, obj[key]);
    }, {});
}
 ...
 * @return {Array} 结果
 */
function exclude(arr, ignores, keys) {
    var fn = void 0;

    if (keys) {
        fn = function fn(obj) {
            return ignores.every(function (ignore) {
                return !equal(obj, ignore, keys);
            });
        };
    } else {
        fn = function fn(obj) {
            return ignores.indexOf(obj) === -1;
        };
    }
...
function flatten(list) {
    return list.reduce(function (res, arr) {
...
function get(obj) {
    ...
    if (arr.length === 1) {
        arr = arr[0].split(/[\.\[\]]/).filter(function (str) {
            return str !== '';
        });
    }
...
function ensureArray(arr) {
    return Array.isArray(arr) ? arr : arr === undefined ? [] : [arr];
}
...
function is(proto, obj) {
    return obj === proto || obj != null && obj.constructor === proto || proto.name === getPrototype(obj);
...
function chunk(arr, size) {
    var output = [];
    for (var i = 0, max = arr.length; i < max; i += size) {
...
function toArray(obj) {
    switch (getPrototype(obj)) {
        ...
        case 'Map':
            return Array.from(obj).map(function (keyVal) {
                return keyVal[1];
            });
        default:
            break;
...
function toObject(obj) {
    return Array.from(obj).reduce(function (item, _ref) {
        var _ref2 = (0, _slicedToArray3.default)(_ref, 2),
            key = _ref2[0],
            val = _ref2[1];

        return set(item, key, val);
    }, {});
}
...
function classify(list, fn) {
    return list.reduce(function (res, info) {
        var type = fn(info);
        res[type] = res[type] || [];
        res[type].push(info);
        return res;
